...
src/librewxr/
  ...
  sources/                                # Per-source packages (auto-discovered)
    ...
    world/rrqpe/                          # NOAA Enterprise Rain Rate (global observed-precip radar region)
...
- **Multi-region:** US (USCOMP, AKCOMP, HICOMP, PRCOMP, GUCOMP), Canada (CACOMP), Central America (SVCOMP), Europe (OPERA + ITCOMP — Italy via DPC), Taiwan (TWCOMP), SE Asia (MYPENINSULAR, MYEAST), plus RRQPE — the always-on global observed radar region (NOAA Enterprise Rain Rate GLB-5, 60S-70N band) that fills only pixels no finer radar region claims as the bottom compositing tier and joins nowcast extrapolation like any region
...
- **Tile bundles:** `GET /v2/radar/{ts}/bundle/{size}/{z}/{x_min}/{y_min}/{x_max}/{y_max}/{color}/{smooth_snow}.{ext}` renders every tile of one frame within an inclusive tile rectangle and returns them in one `LWXB` container (`librewxr/tiles/bundle.py`: 12-byte header — magic `LWXB` + version + uint32 manifest length — then a JSON manifest listing `{x,y,o,n}` per tile, then the concatenated PNG/WebP payloads; fully transparent tiles are omitted). No compression layer — the payloads are already compressed, so the client unpacks by slicing the `ArrayBuffer`. `build_tile_bundle` reuses `compute_tile_geometry` / `present_tile` and the same `TileCache` keys as `radar_tile`, so the single-tile and bundle paths warm each other. Gated by `LIBREWXR_BUNDLE_ENABLED` (503 when off) and capped at `LIBREWXR_BUNDLE_MAX_TILES` (default 256) rectangle area. `examples/bundle-benchmark.html` compares it against per-tile fetching.
